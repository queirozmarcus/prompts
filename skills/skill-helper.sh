#!/bin/bash
# Skills Helper - Utilitário para gerenciar Claude Code skills
...
list_skills() {
    echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
    echo -e "${GREEN}  📚 Claude Code Skills${NC}"
    echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
    echo ""
...
show_skill() {
    ...
    if [ -f "$skill_path/CLAUDE.md" ]; then
        cat "$skill_path/CLAUDE.md"
    else
        echo -e "${YELLOW}No CLAUDE.md found for this skill${NC}"
    fi
}
...
search_skills() {
    ...
    grep -r -i --include="CLAUDE.md" "$query" "$SKILLS_DIR" 2>/dev/null | while IFS=: read -r file content; do
...
validate_skill() {
    ...
    local file="$skill_path/CLAUDE.md"
    if [ ! -f "$file" ]; then
        echo -e "${YELLOW}No CLAUDE.md found for skill '$skill'${NC}"
        return 1
    fi

    echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
    echo -e "${GREEN}  ✅ Validating: $skill${NC}"
    echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
    echo ""
...
new_skill() {
    ...
    local file="$full_path/CLAUDE.md"
...
- Describe how Claude should respond
...
case "$1" in
    ...
    new)
        ...
        ;;
    help|--help|-h)
        echo "Skills Helper - Manage Claude Code Skills"
        echo ""
        echo "Usage:"
