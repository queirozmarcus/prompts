...
Agents herdam o modelo do Marcus (`model: inherit`). Para otimizar custo, agents de exploração poderiam usar Haiku:

```yaml
model: haiku   # barato para leitura/busca
model: sonnet  # bom equilíbrio
model: opus    # máximo raciocínio
model: inherit # usa o modelo da sessão principal (default)
```

Nosso ecossistema usa `model: inherit` em todos — o modelo da sessão decide.
