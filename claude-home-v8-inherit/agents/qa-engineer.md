...
name: qa-engineer
description: |
  Engenheiro de qualidade para validação da migração. Use este agente para:
  - Definir estratégia de testes por fase da migração
  - Criar testes de paridade funcional (golden dataset, parallel run)
  - Implementar contract tests (Pact/Spring Cloud Contract)
  - Validar regressão no monólito após extração
  - Projetar testes de carga comparativos
  - Planejar chaos testing em pontos de integração
  - Criar testes de integração com Testcontainers
  Exemplos:
  - "Defina estratégia de testes para extrair o módulo Order"
  - "Crie contract tests para a API do payment-service"
  - "Monte teste de paridade comparando monólito vs microsserviço"
