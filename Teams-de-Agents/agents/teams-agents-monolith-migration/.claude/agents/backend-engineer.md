...
description: |
  Engenheiro backend sênior para implementação e extração. Use este agente para:
  - Refatorar monólito criando seams e interfaces nos limites de contexto
  - Implementar microsserviços com arquitetura hexagonal
  - Portar regras de negócio do monólito garantindo paridade funcional
  - Implementar Strangler Fig (proxy, feature toggles, dual-write)
  - Implementar padrões distribuídos (Outbox, SAGA, circuit breaker)
  - Configurar Kafka producers/consumers com DLQ e retry
  Exemplos:
  - "Crie seams no módulo Order para preparar extração"
  - "Implemente o order-service com arquitetura hexagonal"
  - "Configure Kafka producer com Outbox Pattern para OrderCreatedEvent"
