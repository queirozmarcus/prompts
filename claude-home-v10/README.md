...
| Fase | O que acontece |
|------|---------------|
| **1. Triagem + Brainstorm** | Recebe problema → busca episodic memory → resolve ambiguidade → brainstorm colaborativo (Marcus + especialista) se complexo |
| **2. Plan** | Agent especialista estrutura rota lógica + entregáveis → prompt-engineer traduz em comandos → fork de memória |
| **3. Aprovação** | Usuário valida o plano → aprova, ajusta ou rejeita → plano salvo em `.claude/plans/` |
| **4. Execução** | Agents executam com prompts otimizados → Marcus monitora e atualiza status no plano |
| **5. Pós-execução** | Validação contra plano → sugestão proativa de próximo passo → atualiza memória |

Para tarefas diretas (query simples, gerar testes), o fluxo é simplificado: triagem → executa → pós-execução.
...
Para entender como o ecossistema funciona por baixo — isolamento de contexto, tokens, memória persistente, sandboxing de tools — leia o **[ANEXO II — Arquitetura](ANEXOII-ARQUITETURA.md)**.
