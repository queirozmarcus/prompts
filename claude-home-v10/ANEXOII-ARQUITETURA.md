...
### Episodic Memory — Recall de Longo Prazo

Além da memória por agent (MEMORY.md), o ecossistema usa **episodic-memory** para busca semântica em todas as conversas anteriores.
...
│                                                           │
│  ┌─── CLAUDE.md ─────────────────────────────────────┐   │
│  │ Instruções estáticas (sempre carregadas)           │   │
│  └───────────────────────────────────────────────────┘   │
│                                                           │
│  ┌─── Agent Memory (MEMORY.md) ──────────────────────┐   │
│  │ Conhecimento curado por agent (patterns, decisões) │   │
│  │ 13 agents · user scope ou project scope            │   │
│  │ Primeiras 200 linhas injetadas no system prompt    │   │
│  └───────────────────────────────────────────────────┘   │
│                                                           │
│  ┌─── Episodic Memory (SQLite + vector) ─────────────┐   │
│  │ TODA conversa anterior indexada semanticamente     │   │
│  │ Busca: "como resolvi X?" → encontra a sessão      │   │
│  │ Arquivado automaticamente via SessionStart hook    │   │
│  │ Haiku subagent gerencia context bloat              │   │
│  │ Acesso: MCP tool (mcp__episodic-memory)            │   │
│  └───────────────────────────────────────────────────┘   │
│                                                           │
│  ┌─── Skills Passivas (28) ──────────────────────────┐   │
│  │ Boas práticas carregadas por contexto de domínio   │   │
│  └───────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────┘
```
...
| Camada | O que guarda | Quando consulta | Quem gerencia |
|--------|-------------|-----------------|---------------|
| CLAUDE.md | Regras do projeto | Sempre (auto-load) | Você |
| Agent Memory | Patterns, decisões do agent | Início de cada invocação | O agent |
| Episodic Memory | Tudo que aconteceu (conversas) | Quando precisa de recall | Claude + Haiku subagent |
| Skills | Boas práticas do domínio | Quando contexto é relevante | Auto-discovery |
...
**Permissões necessárias em settings:**
```json
{
  "permissions": {
    "allow": ["mcp__episodic-memory:*"]
  }
}
```

**Componentes:**
- **Hook SessionStart** → arquiva conversas anteriores automaticamente
- **SQLite + sqlite-vec** → armazena embeddings vetoriais das conversas
- **MCP server** → expõe busca semântica como tool
- **Skill passiva** → ensina Claude quando buscar na memória
- **Haiku subagent** → lê resultados de busca sem poluir seu contexto
...
## Marcus Workflow — 5 Fases

![Workflow completo](marcus-workflow-v10.svg)

### Fluxo de Decisão na Fase 1 (Triagem)
...
Busca episodic memory
    │
    ▼
┌─ Ambíguo? ──────── Sim → Pergunta UMA coisa → loop
│    │
│   Não
│    │
│    ▼
└─ Complexo? ─────── Não → Atalho (executa direto sem brainstorm)
     │
    Sim
     │
     ▼
  Brainstorm (Marcus + Especialista)
     │
     ▼
  Fase 2: Plan
```
...
Planos aprovados são salvos em `.claude/plans/`:

```
.claude/plans/
├── 2026-03-19-billing-service.md       # Status: CONCLUÍDO ✅
├── 2026-03-15-notification-service.md  # Status: CONCLUÍDO ✅
└── 2026-03-10-migration-payment.md     # Status: EM EXECUÇÃO 🔄
```

Cada plano contém: contexto do brainstorm, etapas com agent/command, entregáveis, riscos, e status atualizado durante execução. Servem como rastreabilidade e permitem retomada após interrupção.
