...
show_skill() {
    ...
    if [ -f "$skill_path/GEMINI.md" ]; then
        ...
        echo -e "${YELLOW}No GEMINI.md found for this skill${NC}"
    fi
}
...
search_skills() {
    ...
    grep -r -i --include="GEMINI.md" "$query" "$SKILLS_DIR" 2>/dev/null | while IFS=: read -r file content; do
...
validate_skill() {
    ...
    if [ ! -f "$file" ]; then
        echo -e "${YELLOW}No GEMINI.md found for skill '$skill'${NC}"
        return 1
    fi
